...
delete_disabled() ->
    ...
    delete_disabled_loop(ets:first(?USERDB_RAM)),
    ets:safe_fixtable(?USERDB_RAM, false).

delete_disabled_loop('$end_of_table') -> ok;
delete_disabled_loop(Id) ->
    case ets:lookup(?USERDB_RAM, Id) of
        [#user{enabled = false}] -> delete_user(Id);
        _ -> ok
    end,
    delete_disabled_loop(ets:next(?USERDB_RAM, Id)).
